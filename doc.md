...
```lua
function yalu.ensure.arg(index: number, value: any)
  -> yalu.ensure.argValidator
```
...
```lua
(method) yalu.ensure.argValidator:is(expectedType: "boolean"|"function"|"nil"|"number"|"string"...(+3))
  -> yalu.ensure.baseValidator
```
...
```lua
(method) yalu.ensure.argValidator:isBoolean()
  -> yalu.ensure.booleanValidator
```
...
```lua
(method) yalu.ensure.baseValidator:isFalsy()
  -> yalu.ensure.baseValidator
```
...
```lua
(method) yalu.ensure.argValidator:isFunction()
  -> yalu.ensure.functionValidator
```
...
```lua
(method) yalu.ensure.argValidator:isNumber()
  -> yalu.ensure.numberValidator
```
...
```lua
(method) yalu.ensure.argValidator:isOneOfTypes(..."boolean"|"function"|"nil"|"number"|"string"...(+3))
  -> yalu.ensure.baseValidator
```
...
```lua
(method) yalu.ensure.argValidator:isString()
  -> yalu.ensure.stringValidator
```
...
```lua
(method) yalu.ensure.argValidator:isTable()
  -> yalu.ensure.tableValidator
```
...
```lua
(method) yalu.ensure.baseValidator:isTruthy()
  -> yalu.ensure.baseValidator
```
...
```lua
(method) yalu.ensure.argValidator:isUserdata()
  -> yalu.ensure.userdataValidator
```
...
```lua
(method) yalu.ensure.base:raiseConstraintError(valueExpected: string)
```
...
```lua
(method) yalu.ensure.base:raiseError(expected: string)
```
...
```lua
(method) yalu.ensure.base:raiseTypeError(typeExpected: string)
```
...
```lua
(method) yalu.ensure.baseValidator:validate(condition: boolean|fun(value: any):boolean)
  -> yalu.ensure.baseValidator
```
...
```lua
(method) yalu.ensure.booleanValidator:isFalse()
  -> yalu.ensure.booleanValidator
```
...
```lua
(method) yalu.ensure.booleanValidator:isTrue()
  -> yalu.ensure.booleanValidator
```
...
```lua
(method) yalu.ensure.numberValidator:isNegative()
  -> yalu.ensure.numberValidator
```
...
```lua
(method) yalu.ensure.numberValidator:isNonNegative()
  -> yalu.ensure.numberValidator
```
...
```lua
(method) yalu.ensure.numberValidator:isPositive()
  -> yalu.ensure.numberValidator
```
...
```lua
(method) yalu.ensure.numberValidator:isZero()
  -> yalu.ensure.numberValidator
```
...
```lua
(method) yalu.ensure.stringValidator:isEmpty()
  -> yalu.ensure.stringValidator
```
...
```lua
(method) yalu.ensure.stringValidator:isNotEmpty()
  -> yalu.ensure.stringValidator
```
...
```lua
(method) yalu.ensure.tableValidator:hasKey(key: any)
  -> yalu.ensure.tableValidator
```
...
```lua
(method) yalu.ensure.tableValidator:hasKeys(...any)
  -> yalu.ensure.tableValidator
```
...
```lua
(method) yalu.ensure.tableValidator:hasValue(value: any)
  -> yalu.ensure.tableValidator
```
...
```lua
(method) yalu.ensure.tableValidator:isEmpty()
  -> yalu.ensure.tableValidator
```
...
```lua
(method) yalu.ensure.tableValidator:isNotEmpty()
  -> yalu.ensure.tableValidator
```
...
```lua
function yalu.fs.copyFile(source: string, destination: string)
  -> success: boolean
...
@*param* `destination` — The path where the copy will be created.

@*return* `success` — True on success.

@*return* `errorMessage` — False and an error message on failure.

## doesDirExist


```lua
function yalu.fs.doesDirExist(path: string)
  -> exists: boolean
```
...
@*return* `exists` — True if the path exists and is a directory, false otherwise.

## doesFileExist


```lua
function yalu.fs.doesFileExist(fileName: string)
  -> exists: boolean
```
...
@*return* `exists` — True if the file exists and can be opened, false otherwise.

## getDirectoryList


```lua
function yalu.fs.getDirectoryList(directory: string, filter?: string)
  -> dirList: string[]
```
...
@*param* `directory` — The root directory path to search in.

@*param* `filter` — A Lua pattern to filter the resulting directory paths.

@*return* `dirList` — An array of normalized directory paths matching the filter.

## getFileList


```lua
function yalu.fs.getFileList(directory: string, filter: string)
  -> fileList: string[]
```
...
@*param* `filter` — A Lua pattern to filter the resulting file paths.

@*return* `fileList` — An array of normalized file paths matching the filter.

## joinPaths


```lua
function yalu.fs.joinPaths(...string|number)
  -> string
```

Joins multiple path components into a single normalized path.

@*param* `...` — Path components to join (e.g., dir, sub-dir, fileName).
...
```lua
function yalu.fs.normalizePath(path: string)
  -> string
```

Normalizes a path string to use single forward slashes.

@*param* `path` — The raw path to be cleaned.
...
```lua
function yalu.fs.readFile(fileName: string)
  -> content: string|nil
  2. errorMessage: string|nil
```
...
@*return* `content` — The file contents, or nil if the file could not be opened.

@*return* `errorMessage` — The error message if the file could not be opened.
...
```lua
function yalu.logging.getInstance()
  -> yalu.logging.logger
```
...
```lua
function yalu.logging.new(name: string, level?: yalu.logging.logLevel, outputFunc?: function, outputErrFunc?: function)
  -> logger: yalu.logging.logger
```

Create a new logger instance or return the existing one.

@*param* `name` — Logger name used in the log prefix (defaults to the source filename).

@*param* `level` — The initial log level.

@*param* `outputFunc` — Optional function to handle log output (defaults to print).

@*param* `outputErrFunc` — Optional function to handle error output (defaults to error).

@*return* `logger` — The newly created logger instance or the existing one.


---

# yalu.logging.logLevel
...
```lua
function yalu.logging.logger.debug(value: any, ...any)
```
...
```lua
function yalu.logging.logger.error(value: any, ...any)
```
...
```lua
function yalu.logging.logger.fatal(value: any, ...any)
```
...
```lua
function yalu.logging.logger.info(value: any, ...any)
```
...
```lua
function yalu.logging.logger.setLevel(newLevel?: yalu.logging.logLevel)
```
...
```lua
function yalu.logging.logger.trace(value: any, ...any)
```
...
```lua
function yalu.logging.logger.warn(value: any, ...any)
```
...
```lua
function yalu.math.isAlmostEqual(x: number, y: number, epsilon?: number)
  -> isClose: boolean
```
...
@*param* `epsilon` — The maximum allowed difference between x and y.

@*return* `isClose` — Returns true if the difference is less than epsilon.

## round


```lua
function yalu.math.round(x: number, decimalPlaces: number)
  -> The: number
```
...
@*return* `The` — number truncated to decimalPlaces.
...
```lua
function yalu.string.stringToTable(str: string, separator: string)
  -> table: table
```
...
@*param* `separator` — The character(s) to use as separators (treated as a character set).

@*return* `table` — A map where keys are tokens and values are their order of appearance.
...
```lua
function yalu.table.deepCopy(orig: table, cache?: table)
  -> copy: any
```
...
@*param* `cache` — Internal table used to track already copied objects.

@*return* `copy` — A completely independent clone of the original table.

## flatten


```lua
function yalu.table.flatten(list: table)
  -> table: table
```
...
@*param* `list` — An array of tables to be merged.

@*return* `table` — A new table containing all merged key-value pairs.

## includes


```lua
function yalu.table.includes(list: table, value: any)
  -> boolean
```
...
```lua
function yalu.table.merge(t1: table<<K>, <V>>, t2: table<<K>, <V>>)
  -> t1: table<<K>, <V>>
```
...
@*param* `t1` — The destination table that will be modified.

@*param* `t2` — The source table containing values to merge.

@*return* `t1` — The modified destination table.
...
```lua
function yalu.utils.getCallerInfo()
  -> level: number
  2. name: string
  3. info: debuginfo
```

Retrieves information about the first stack frame that is outside the current module.<br>
This can be used to report errors with the correct caller context.

@*return* `level` — Stack level suitable for passing to `error`.

@*return* `name` — Name of the calling function, or `<anonymous>` if none.

@*return* `info` — Full debug info table from `debug.getinfo`..

## isWindows


```lua
function yalu.utils.isWindows()
  -> isWindows: boolean
```

Detect whether the current runtime path separator is Windows-style.

@*return* `isWindows` — `true` when running on Windows, otherwise `false`.